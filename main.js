const questions = [
  ...
  {
    question: "Är mors lilla Olle i skogen?",
    type: "radiobutton",
    answers: [
      { text: "Sant", isCorrect: true },
      { text: "Falskt", isCorrect: false },
...
const SCREEN_WELCOME = 0;   //Börja med welcome skärm
const SCREEN_QUESTION = 1;   //sen börja med question skärm
const SCREEN_ANSWERS = 2;     //sen avsluta med answes skärm

//visa första screen i skärmen
...
function showQuestion(questionIndex) {
  ...
  if (questionIndex === questions.length - 1 ) {    //när anvädare kommer till sista knappen så visa Get results button
    nextButton.textContent = "Get results!";
  } else {
    nextButton.textContent = "Next";            //annars visa bara next button
  }
}

function createAnswerOptionHtml(type, text, answerIndex) {
  if (type === "radiobutton") {  //funktionalitet för radiobutton 
    return `
    ...
    `;
  } else { //funktionalitet för checkboxar 
    return `
      <div class="label_cover">
...
//alternativ 1 
  ...
  //alternativ 2 med procent  * questions.length
  /*let scoreColor;
...
//när man klickar på start, reset och startoverbutton  button så kör startQuiz funkation
startButton.addEventListener("click", () => {
  startQuiz();
});
//när man klickar på reset knappen, reset och startoverbutton  button så kör startQuiz funkation

resetButton.addEventListener("click", () => {
  startQuiz();
});
//när man klickar på Take the test again btn knappen, reset och startoverbutton  button så kör startQuiz funkation
...
nextButton.addEventListener("click", () => {
  ...
  let answerInputs =
    questionAnswersContainer.querySelectorAll("[name='answer']");
